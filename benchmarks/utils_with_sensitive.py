...
def evaluation(idx1, idx2, clf, X, y, s, t,
               acc, brier, auc, meo, meo_abs, abseo, sp):
    y_prob = np.squeeze(clf.predict_proba(X=X, s=s), axis=2)
    y_pred = (y_prob[:, 1] > t).astype('int')
    ...
    meo[idx1, idx2], meo_abs[idx1, idx2], abseo[idx1, idx2] = odd_diffs(y, y_pred, s)
...
def MP_tol(df, tolerance, protected_attrs, label_name, use_protected, use_sample_weight, tune_threshold, log, model='gbm', div='cross-entropy', num_iter=10, rand_seed=42, constraint='meo'):
    ...
    abseo = np.zeros((len(tolerance), num_iter))
    ...
    for seed in tqdm(range(num_iter)):
        ...
        # resample 30% with replacement to get test set 
        ...
        if model == 'gbm':
            ...
            # adult 
            clf_YgX = RandomForestClassifier(random_state=rand_seed, n_estimators=15, min_samples_leaf=3, criterion = 'log_loss', bootstrap = False)
            clf_SgX = RandomForestClassifier(random_state=rand_seed, n_estimators=17)
            clf_SgXY = RandomForestClassifier(random_state=rand_seed, n_estimators=17)
            
            # compas
            # clf_YgX = RandomForestClassifier(random_state=rand_seed, n_estimators=17) # will predict Y from X
            # clf_SgX = RandomForestClassifier(random_state=rand_seed, n_estimators=17)  # will predict S from X (needed for SP)
            # clf_SgXY = RandomForestClassifier(random_state=rand_seed, n_estimators=17)  # will predict S from (X,Y)
        else:
...
# utils for the leveraging approach
def odd_diffs_leveraging(y, y_pred, s):
    y0, y1 = y[s==0], y[s==1]
    y_pred0, y_pred1 = y_pred[s==0], y_pred[s==1]
...
def evaluate_leverage(X, y, y_pred, s):
    ...
    abseo = odd_diffs_leveraging(y, y_pred, s)
    sp = statistical_parity_difference(y_pred, s)
    return acc, abseo, deo, sp
...
def leveraging_approach(df, protected_attrs, label_name, use_protected, use_sample_weight=False, log='log.txt', model='gbm', num_iter=10, rand_seed=42):
    ...
    for seed in tqdm(range(num_iter)):
        ...
        if model == 'gbm':
            ...
            # adult
            clf_YgX = RandomForestClassifier(random_state=rand_seed, n_estimators=15, min_samples_leaf=3, criterion = 'log_loss', bootstrap = False)   # will predict Y from X

            # compas
            # clf_YgX =  RandomForestClassifier(random_state=rand_seed, n_estimators=17)
        ...
        base_acc[seed], base_abseo[seed], base_deo[seed], base_sp[seed] = evaluate_leverage(X_test, y_test, clf_YgX.predict(X=X_test), s_test)
        ...
        leverage_acc[seed], leverage_abseo[seed], leverage_deo[seed], leverage_sp[seed] = evaluate_leverage(X_test, y_test, y_test_leveraged, s_test)

        log.write(' Base: Acc {:.2f}, meo {:.2f}, deo {:.2f}, sp {:.2f}\n'.format(base_acc[seed], base_abseo[seed], base_deo[seed], base_sp[seed]))
        log.write(' Leve: Acc {:.2f}, meo {:.2f}, deo {:.2f}, sp {:.2f}\n'.format(leverage_acc[seed], leverage_abseo[seed], leverage_deo[seed], leverage_sp[seed]))
