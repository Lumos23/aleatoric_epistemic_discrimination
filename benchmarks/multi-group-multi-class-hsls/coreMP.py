...
def admm_tf(G,y,rho=2,div = 'kl', max_iter = 1000, eps1=1e-7, eps2=1e-2, eps3=1e-7):
    ...
    n,c,k = G.shape
    reg = .0001/ np.sqrt(n) #for both adult and compas
    ...
    ln = list(iter(lambda: tuple(islice(it, BATCH_SIZE)), ())) 
...
#         Gl_tf_slice.assign(tf.linalg.matmul(G_tf[ix[0]:ix[-1]+1,:,:],l_tf))
    ...
    # if report:
        if (ix == max_iter-1):
            ...
            print('Max infeasibility: '+str(max_infeasibility))
...
def predict(l,G,y,div='kl'):
    ...
    # compute v
    v = G @ l
#     print("G In MP predict:", G)
#     print("l In MP predict:", l)
